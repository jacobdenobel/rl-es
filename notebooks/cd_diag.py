...
def draw_cd_diagram(df_perf=None, alpha=0.05, title=None, labels=False, **kwargs):
    """
    Draws the critical difference diagram given the list of pairwise classifiers that are
    significant or not
    """
    p_values, average_ranks, _ = post_hoc_test(df_perf=df_perf, alpha=alpha, **kwargs)
...
def post_hoc_test(
    ...
    """
    echo(pd.unique(df_perf[classifier_name]))
    ...
    if friedman_p_value >= alpha:
        ...
        echo("the null hypothesis over the entire classifiers cannot be rejected")
        exit()
    ...
    if test == "wilcoxon":
        ...
        ney_fried = sp.posthoc_nemenyi_friedman(
            df_perf,
            y_col=accuracy,
            block_col=dataset_name,
            group_col=classifier_name,
            melted=True,
        )
        for i in range(m - 1):
            classifier_1 = classifiers[i]
            for j in range(i + 1, m):
                classifier_2 = classifiers[j]
                p_value = ney_fried[classifier_1][classifier_2]
                p_values.append((classifier_1, classifier_2, p_value, False))
                
    else:
        raise ValueError('only wilcoxon or nemenyi are supported values for test')
    ...
    for i in range(k):
        if correction == 'holm':
            ...
            new_alpha = alpha / len(p_values)
        else:
            raise ValueError("only holm or bonferroni are allowed for correction")
...
if __name__ == "__main__":
    ...
    data = pd.read_pickle("mdata_atari.pkl")
    draw_cd_diagram(
        data,
        alpha=0.05,
        title='Atari (Wilcoxon)',
        labels=True,
